...
def main():
    ...
    with tab_list:
        # Adjusted to exactly 16 rows per view
        per_page = 16
        ...
        if data:
            ...
            # CSS hack to further reduce font size inside the dataframe
            st.markdown("<style>div[data-testid='stDataFrame'] { font-size: 10px !important; }</style>", unsafe_allow_html=True)
            ...
            st.dataframe(
                ...
                hide_index=True,
                height=570 # Optimized height for exactly 16 rows with 10px font
            )
